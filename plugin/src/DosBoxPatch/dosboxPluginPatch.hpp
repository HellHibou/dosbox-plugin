...
private:
	...
	static vm::type::InterruptHandle        interrupts [256];
...
public:
	...
	static void start();
	static void unload();
	static bool interrupt(Bitu num);
};

inline bool DosBoxPluginManager::interrupt(Bitu num) {
	if (interrupts[num] == NULL) return false;
	return interrupts[num]((vm::type::Regs *)&cpu_regs);
}
